...
UA = UserAgent()
KEY = "91cfe7f4-4a50-432f-9a9d-e95e00c05adf"
RE = {
    "tit": re.compile(
...
def update_db(args, thread_objs, populate=False):
    ...
    with httpx.Client(timeout=120, headers={"apiKey": KEY}) as cl:
        ...
        try:
            resp = cl.get(
                f"{URL['nvd'].format('cpes', 0)}&resultsPerPage=1{extra}".replace("+", "%2B")
            )
            #print(f"{URL['nvd'].format('cves', 0)}&resultsPerPage=1{extra}".replace("+", "%2B"))
        ...
        try:
            ...
            os._exit(1)
        resp = cl.get(
            f"{URL['nvd'].format('cves', 0)}" f"&resultsPerPage=1{extra}".replace("+", "%2B")
        )
        try:
            #print(f"{URL['nvd'].format('cves', 0)}" f"&resultsPerPage=1{extra}")
